...
impl RuntimeIntrospector for RubyIntrospector {
    ...
    fn extract_variables(&self, proc: &HeadlessProcess) -> Result<Vec<Variable>, RevError> {
        let _ruby_base = find_ruby_base(proc.pid)?;
        Ok(Vec::new())
    }
...
#[cfg(target_os = "linux")]
fn find_ruby_base(pid: u32) -> Result<(String, u64), RevError> {
    use std::fs::File;
    ...
    for line_result in reader.lines() {
        ...
        if line.contains("/ruby") || line.contains("libruby") {
    ...
    Err(RevError::UnsupportedRuntime(
        "ruby binary/library not found in maps".to_string(),
    ))
}
...
impl RuntimeIntrospector for RubyIntrospector {
    ...
    fn spawn_headless(
        ...
        Err(RevError::UnsupportedPlatform(
            "Ruby Introspection is only supported on Linux".to_string(),
        ))
    }
